...
if grep -Eq 'overlayroot=tmpfs:recurse=0|boot=overlay' /proc/cmdline 2>/dev/null; then
  pass "overlay enabled in running kernel cmdline"
elif cmdline_has "overlayroot=tmpfs:recurse=0" || cmdline_has "boot=overlay"; then
  pass "overlay enabled in boot cmdline"
else
  warn "overlay boot args not enabled (maintenance mode?)"
fi
