...
EOF
  ...
  ts=$(date -Is)
  if [[ -z "$sample" ]]; then
    alert "board unreachable at $BOARD"
    sleep "$INTERVAL"
    continue
  fi
  ...
  [[ "$health" == "ok:0" ]]            || alert "health=$health"
  [[ "${failed:-1}" -eq 0 ]]           || alert "failed units: $failed"
  [[ "${bufio:-1}" -eq 0 ]]            || alert "Buffer I/O errors in dmesg: $bufio"
  [[ "${age:-999}" -lt 180 ]]          || alert "sync stalled: last usage write ${age}s ago"
  [[ "${ovl:-100}" -lt 50 ]]           || alert "overlay at ${ovl}%"
  [[ "${temp:-99}" -lt 75 ]]           || alert "SoC temp ${temp}C"
  [[ "${mem:-0}" -gt 500 ]]            || alert "MemAvailable ${mem}MB"
  [[ "${rot:-}" != "panic" ]]          || alert "rotator state: panic"

  # Capture progress: while the writer produces files, the mirror must grow.
  if [[ "$raw" =~ ^[0-9]+$ ]]; then
    if [[ "$raw" -eq "$prev_raw" && "$writer" -gt 0 ]]; then
      raw_stall=$((raw_stall + 1))
      [[ $raw_stall -ge 5 ]] && alert "mirror not growing: raw=$raw for $raw_stall samples (writer at $writer files)"
    else
      raw_stall=0
    fi
    prev_raw=$raw
